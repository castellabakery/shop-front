...
import {Checkbox, Descriptions, Radio, Form, Input} from "antd";

import '../BuyerDetail.css';
import moment from "moment";

const BuyerApproveDetailDescription_Medicine = function (props) {
                        ...
                        >
                            {initData.result.hasOwnProperty('fileList') && wInitData.result.hasOwnProperty('fileList') ?
                                (initData.result.fileList.length > 0 ?
                                    (initData.result.fileList.filter(item => item.fileType === 6).length > 0 ? <font style={{color:"red"}}>파일보기</font> : '파일보기') : (wInitData.result.fileList.length > 0 ? '파일보기' : '파일없음')) : '파일없음'
                            }
                        </a>
                    </Descriptions.Item>
                    ...
                    <Descriptions.Item label="가입신청일자" span={2}>
                        {initData.result.hasOwnProperty('createdDatetime') && wInitData.result.hasOwnProperty('buyer') ?
                            (initData.result['createdDatetime'] != wInitData.result['createdDatetime'] ?
                                    ...
                                    moment(wInitData.result['createdDatetime']).format('YYYY-MM-DD HH:mm:ss')
                            ) :
                        ''}
                    </Descriptions.Item>
                </Descriptions>
...
export default BuyerApproveDetailDescription_Medicine;