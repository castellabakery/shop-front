...
const BuyerSignupCompany = function () {
    ...
    const goNext = (ci, sMobileNo, sName) => {
        navigate("/login/signup/hospital/step3", {
            state:{
                sConnInfo: ci,
                sMobileNo: sMobileNo,
                sName: sName
    ...
    return (
        <div>
            <Content>
                ...
                <Row>
                    <Col span={"24"}>
                        <Button href="/login/signup/hospital/step1" className='btn-color02-2 btn-huge btn-fix1'  style={{width: "300px"}} >취소</Button> <Button type="primary" onClick={e => {goNext(ci, sMobileNo, sName)}} disabled={checkItems.length === checkboxList.length ? false : true}  className='btn-color02 btn-huge' style={{width: "300px"}}>다음단계</Button>
                    </Col>
                </Row>
            </Content>
