...
import {UploadOutlined, UserDeleteOutlined} from "@ant-design/icons";
...
const BuyerSignupCompany = function () {
    ...
    const onSubmit = useCallback((values) => {
        ...
        .then((res) => {
            if(res.data.data.seq){
                ...
                // })
                navigate("/login/signup/company/step4");
            } else{
                // 회원가입 롤백
            }
        }).catch((error) => {
    ...
    const isRegularExpression = (type, testVal) => {
        let rgx, returnBoolean = false;

        if(type == 'numNeng'){
            //영문,숫자만 가능
            rgx = /^[0-9a-zA-Z][0-9a-zA-Z_-]+[0-9a-zA-Z]{1,10}$/g;
        }else if(type == 'pwd01'){
            //영문 + 숫자 + 특수문자 혼합, 8~16이하
            rgx = /(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^*()\-_=+\\\|\[\]{};:\'",.<>\/?]).{8,16}$/;
        }else if(type == 'blank'){
            //공백
            rgx = /\s/g;
        }
    ...
    const onChangePassword = ((e) => {
        ...
        // let regex_check = isRegularExpression('pwd01', e);
        // if (!regex_check) {
        //     Message.error('비밀번호 규칙이 어긋납니다. 확인해주세요.');
        //     return;
        // }
                    ...
                    <Col>
                        ...
                        >
                            ...
                            <div className='align-center spacerTop'>
                                <Button href="/login/signup/company/step1" htmlType={"submit"} className={"login-form-button btn-color02-2 btn-huge btn-fix1"} style={{width : "200px"}}>취소</Button> <Button type={"primary"} htmlType={"submit"}  className={"login-form-button btn-color02 btn-huge btn-fix1"} style={{width : "200px"}}>회원가입</Button>
                            </div>
                        </Form>
                    </Col>
                </Row>
